...
As duas frentes são complementares. A **sonda** (Fases 1–2) mede o
comportamento do DiffusionPen original e usa o patch aditivo. O **fine-tune**
no BRESSAY usa as correções de `diffusionpen_mods/`, que são substituições de
arquivo, não um patch.

> **Antes de treinar em qualquer GPU, rode `diagnostico/`.** A placa usada no
> desenvolvimento do fine-tune (RX 6600 XT) computa este modelo de forma
> numericamente instável, e isso invalidou todos os modelos treinados nela.
> Os números estão em `diagnostico/README.md`; o estado atual, em `STATUS.md`.
...
Com Nix, o ambiente já vem pronto — escolha o shell da sua placa:

```bash
nix develop .#cuda      # NVIDIA (recomendado para treinar)
nix develop .#rocm      # AMD
nix develop .#cpu       # sem GPU (métrica e referência numérica)
```

Para o fine-tune no BRESSAY, siga o `STATUS.md`. O roteiro abaixo é o da sonda
(Fases 1–2), com venv:
