...
class WordLineDataset(Dataset):
    ...
    def __finalize__(self):
        ...
        # Indices pre-computados para o __getitem__ nao precisar varrer o
        # dataset inteiro duas vezes por amostra. Com 74.882 amostras aquilo
        # dava ~11 bilhoes de comparacoes por epoca, e cada varredura montava
        # uma lista com quase todo o dataset dentro de cada worker.
        self.indices_por_wid = {}
        self.indices_longos_por_wid = {}
        self.indices_longos = []
        for i, d in enumerate(data):
            self.indices_por_wid.setdefault(d[2], []).append(i)
            if len(d[1]) > 3:
                self.indices_longos.append(i)
                self.indices_longos_por_wid.setdefault(d[2], []).append(i)
        if not self.indices_longos:
            self.indices_longos = list(range(len(data)))
    ...
    def _imagem(self, index):
        '''
        Resolve a posicao 0 de uma entrada de self.data para uma PIL Image.
        Datasets que guardam a imagem ja carregada (IAM) passam direto; os que
        guardam so o caminho (BRESSAY) abrem aqui, no worker, o que mantem o
        consumo de RAM constante em vez de proporcional ao tamanho do split.
        '''
        entrada = self.data[index][0]
        ...
        # Mesma regra de reducao que o main_loader aplicava quando carregava
        # todas as imagens de uma vez.
        if img.height < 64 and img.width < 256:
            return img
        return image_resize_PIL(img, height=img.height // 2)

    def __getitem__(self, index):

        img = self._imagem(index)
        ...
        # Positivo: outra amostra do mesmo escritor. Preferimos transcricoes
        # com mais de 3 caracteres, mas caindo de volta para todas as amostras
        # do escritor quando nao existir nenhuma -- no BRESSAY, recortado em
        # palavras, 50 dos 647 escritores do train nao tem UMA palavra com
        # mais de 3 letras (o portugues e cheio de "de", "do", "que"), e o
        # filtro rigido deixava a lista vazia: random.choice([]) -> IndexError.
        pos_idx = self.indices_longos_por_wid.get(wid) or self.indices_por_wid[wid]
        positive = self._imagem(random.choice(pos_idx))

        # random.choices (com reposicao) em vez de random.sample, que exige
        # k amostras distintas e estouraria em escritores com menos de 5.
        style_images = [self._imagem(i) for i in random.choices(pos_idx, k=5)]

        # Negativo: sorteia uniformemente entre as amostras elegiveis e rejeita
        # as do proprio escritor. Mantem a distribuicao do codigo original
        # (uniforme por amostra, nao por escritor) sem varrer o dataset.
        for _ in range(100):
            neg_i = random.choice(self.indices_longos)
            if self.data[neg_i][2] != wid:
                break
        negative = self._imagem(neg_i)
...
class BressayDataset_style(WordLineDataset):
    # Assinatura idêntica à do IAMDataset_style
    def __init__(self, basefolder, subset, segmentation_level, fixed_size, transforms):
        super().__init__(basefolder, subset, segmentation_level, fixed_size, transforms)
        self.setname = 'BRESSAY'
    ...
    def main_loader(self, subset, segmentation_level) -> list:
        ...
        for i, line in enumerate(lines):
            ...
            img_rel_path, writer_id, transcr = line.split('\t')
            writer_id = int(writer_id)
            full_img_path = os.path.join(self.images_root, img_rel_path)
            
            if i % 20000 == 0:
                print(f'BRESSAY [{subset}]: lendo indice [{i}/{len(lines)} ({100.*i/len(lines):.0f}%)]')

            transcr = transcr.replace(" ", "")

            # Guarda o CAMINHO na posicao 0, nao a imagem: o _imagem() da
            # classe pai abre o arquivo no worker. Carregar as 74.882 imagens
            # de uma vez custava alguns GB no processo principal e voltava a
            # ser copiado em cada worker do DataLoader (o refcount do CPython
            # toca a pagina, entao copy-on-write nao ajuda) -- estourava 32GB.
            data += [(full_img_path, transcr, writer_id, full_img_path)]

        print(f'BRESSAY [{subset}]: indice lido. {len(data)} amostras (lazy load).')
        return data
...
def val_epoch_mixed(val_loader, model, criterion_triplet, criterion_classification, optimizer, device, args):
    ...
    accuracy = n_corrects/total
    print("Validation Loss: {:.4f}".format(running_loss/len(val_loader)))
    print("Validation Triplet Loss: {:.4f}".format(running_triplet/len(val_loader)))
    print("Validation Accuracy: {:.4f}".format(accuracy*100))
    # Seleciona checkpoint pelo triplet loss, nao pela soma. Os escritores de
    # train e val sao disjuntos, entao a cabeca de classificacao nunca viu as
    # classes do val: aquele termo e inaprendivel por construcao e, somado,
    # dominaria a metrica e faria a escolha do "melhor" modelo virar ruido. O
    # triplet e justamente o que interessa para o uso downstream -- mede se o
    # embedding de estilo separa escritores que o encoder nunca viu.
    return running_triplet/total
...
def main():
    ...
    if args.dataset == 'iam':
        ...
        myDataset = BressayDataset_style
        ...
        # Como o Bressay já tem validação isolada, instanciamos direto
        train_data = myDataset(dataset_folder, 'train', 'word', fixed_size=(1 * 64, 256), transforms=train_transform)
        val_data = myDataset(dataset_folder, 'val', 'word', fixed_size=(1 * 64, 256), transforms=val_transform)
        ...
        style_classes = 1000 # Total de escritores original
    ...
    if args.model == 'mobilenetv2_100':
        ...
        # Correção: Usar a classe Mixed_Encoder que devolve (logits, features)
        if args.mode == 'mixed':
